...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        scale = self.squeeze_excitation(x)
        return x * scale
...
class ResidualBlock(nn.Module):
    ...
    def __init__(self, dim_in, dim_out):
        ...
        self.main = nn.Sequential(
            ...
            nn.InstanceNorm2d(dim_out, affine=True, track_running_stats=True),
            SEBlock(dim_out),
            ChannelAttention(dim_out),
            SpatialAttention(dim_out)
            )
...
class ProAttentionBlock(nn.Module):
    """Attention Block module."""
    def __init__(self, in_channels, num_styles):
        super(ProAttentionBlock, self).__init__()
...
class Generator(nn.Module):
    ...
    def __init__(self, conv_dim=64, c_dim=5, repeat_num=6):
        ...
        for i in range(repeat_num):
            ...
            attention_residual_layers.append(ProAttentionBlock(in_channels=curr_dim, num_styles=c_dim)) ##Proposed
        self.attention_residual_blocks = nn.ModuleList(attention_residual_layers)

        # for i in range(repeat_num):
        #     layers.append(ResidualBlock(dim_in=curr_dim, dim_out=curr_dim))   ##self-attention
        #     layers.append(SelfAttentionBlock(in_channels=curr_dim))
...
class CAModule(nn.Module):
    def __init__(self, channel):
        ...
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.conv = nn.Conv2d(channel, channel // 8, 1, bias=False)
        self.relu = nn.ReLU(inplace=True)
        self.fc = nn.Conv2d(channel // 8, channel, 1, bias=False)
...
class Discriminator(nn.Module):
    ...
    def __init__(self, image_size=128, conv_dim=64, c_dim=5, repeat_num=6):
        ...
        for i in range(1, repeat_num):
            ...
            layers.append(SELayer(curr_dim*2))
            layers.append(CAModule(curr_dim*2))
            layers.append(nn.LeakyReLU(0.01))
            curr_dim = curr_dim * 2
